        ...
        <?php else: ?>
            ...
            <div class="order-summary">
                    ...
                    <select id="payment_method" name="payment_method">
                        <?php foreach ($pttt as $id) { ?>
                            <option value="<?php echo $id['id_payment'] ?>"><?php echo $id['payment_method'] ?></option>
                        <?php } ?>
                    </select>

                    <p class="total-price-value">Tổng tiền: <span id="total-price"><?php echo number_format($totalPrice, 0, ',', '.'); ?> đ</span></p>
                    <a href="#" id="checkout-link" class="checkout">THANH TOÁN</a>
                </div>
                <script>
                    document.getElementById('checkout-link').addEventListener('click', function() {
                        var paymentMethod = document.getElementById('payment_method').value;
                        var url = 'index.php?quanly=thanhtoan&payment_method=' + encodeURIComponent(paymentMethod);
